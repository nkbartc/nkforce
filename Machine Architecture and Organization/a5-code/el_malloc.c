...
el_blockhead_t *el_block_below(el_blockhead_t *block){
  el_blockfoot_t *pre_footer =
    PTR_MINUS_BYTES(block, sizeof(el_blockfoot_t));
  if((void *) pre_footer <= (void*) el_ctl.heap_start){
      return NULL;
    }
  el_blockhead_t *lower = el_get_header(pre_footer);
...
el_blockhead_t *el_find_first_avail(size_t size){
  size_t total = size + EL_BLOCK_OVERHEAD;
  el_blocklist_t *avai_list = el_ctl.avail;
  el_blockhead_t *beg = avai_list->beg->next;

  int i = 0;
  while(i < avai_list->length){
    if(total <= beg->size){
      return beg;
    }
    i++;
...
el_blockhead_t *el_split_block(el_blockhead_t *block, size_t new_size){
  ...
  el_blockhead_t *new_used = block;
  el_blockfoot_t *avail_foot = el_get_footer(block);
  el_blockfoot_t *used_foot  = PTR_PLUS_BYTES(block, sizeof(el_blockhead_t) + new_size);
  size_t temp = block->size;


  new_used->size = new_size;
  used_foot->size = new_used->size;

  el_blockhead_t *new_avail =
  PTR_PLUS_BYTES(new_used, EL_BLOCK_OVERHEAD + new_size);
  new_avail->size = temp - (EL_BLOCK_OVERHEAD + new_size);
  avail_foot->size = new_avail->size;
...
void *el_malloc(size_t nbytes){
  if (el_find_first_avail(nbytes) == NULL){
    return NULL;
  }

  el_blockhead_t *demand = el_find_first_avail(nbytes);
  el_remove_block(el_ctl.avail, demand);
  el_blockhead_t *result = el_split_block(demand, nbytes);
  result->state= EL_USED;
  el_add_block_front(el_ctl.used, result);
  el_blockhead_t *avai =
  PTR_PLUS_BYTES(result, result->size + sizeof(el_blockhead_t));
  avai->state= EL_AVAILABLE;
  el_add_block_front(el_ctl.avail, avai);
  return PTR_PLUS_BYTES(result, sizeof(el_blockhead_t));
}
...
void el_merge_block_with_above(el_blockhead_t *lower){
  ...
  el_blockhead_t *new_header = lower;
  el_blockfoot_t *new_footer = el_get_footer(higher);

  el_remove_block(el_ctl.avail, lower);
  el_remove_block(el_ctl.avail, higher);

  new_header->size = EL_BLOCK_OVERHEAD + lower->size + higher->size;
  new_footer->size = new_header->size;

  el_add_block_front(el_ctl.avail, new_header);
...
void el_free(void *ptr){
  el_blockhead_t *target = PTR_MINUS_BYTES(ptr, sizeof(el_blockhead_t));
  target->state = EL_AVAILABLE;
  el_blockhead_t *left = el_block_below(target);
  el_remove_block(el_ctl.used, target);
  el_add_block_front(el_ctl.avail, target);
  el_merge_block_with_above(target);
  //merge left if possible, merge function will take care of it
  el_merge_block_with_above(left);
}
