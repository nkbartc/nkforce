...
def CountingSort(studentList,houseOrYear):
    ...
    c = []
    result = [None] * len(studentList)
    if houseOrYear == 'h':
        for i in range(0, 4):
            c.append(0)

        for j in range(0, len(studentList)):
            c[studentList[j].getIndex('h')] = c[studentList[j].getIndex('h')] + 1

        for i in range(1, 4):
            c[i] = c[i] + c[i-1]

        for j in range(len(studentList) - 1, -1, -1):
            result[c[studentList[j].getIndex('h')] - 1] = studentList[j]
            c[studentList[j].getIndex('h')] = c[studentList[j].getIndex('h')] - 1
        ...
        for j in range(0, len(studentList)):
            c[studentList[j].getIndex('y')] = c[studentList[j].getIndex('y')] + 1

        for i in range(1, 8):
            c[i] = c[i] + c[i-1]

        for j in range(len(studentList) - 1, -1, -1):
            result[c[studentList[j].getIndex('y')] - 1] = studentList[j]
            c[studentList[j].getIndex('y')] = c[studentList[j].getIndex('y')] - 1
