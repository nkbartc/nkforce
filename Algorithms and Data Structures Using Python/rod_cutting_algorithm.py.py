...
def optimizeNuggets(priceList,numNuggets):
    rev = [None] * (numNuggets + 1)
    split = [None] * (numNuggets + 1)
    rev[0] = 0

    for j in range(1, numNuggets + 1):
        cheapest = 999
        for i in range(1, j + 1):
            if cheapest > priceList[i] + rev[j - i]:
                cheapest = priceList[i] + rev[j - i]
                split[j] = i
        rev[j] = cheapest
    result = []
    while(numNuggets > 0):
        result.append(split[numNuggets])
        numNuggets = numNuggets - split[numNuggets]
