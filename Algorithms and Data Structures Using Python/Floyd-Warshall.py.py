import traceback, turtle, math, copy
...
def optimize_hyperspace_routes(star_list,jump_times):
    ...
    if x != None:
        ...
        cycle_object = []
        for x in cycle_path:
            cycle_object.append(star_list[x])
        return list(reversed(cycle_object))
    return dist

    
def setup_prev(weights):
    n = len(weights)
    prev = copy.deepcopy(weights)
    for i in range(n):
        for j in range(n):
            prev[i][j] = None
    for i in range(n):
        for j in range(n):
            if i != j and weights[i][j] < 999:
                prev[i][j] = i
    return prev
